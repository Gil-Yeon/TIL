...
def replaceData(numData): # numData	2차원 정수 배열
    ...
    ###########   여기부터 코딩 (1) ----------------> 
    retData = numData.copy()
    row_length = len(retData)
    col_length = len(retData[0])
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    ###########   여기부터 코딩 (2) ---------------->
    row_length = len(numData)
    col_length = len(numData[0])
    for i in range(row_length-1):
          for j in range(col_length-1):
                pre_maxSum = maxSum
                maxSum = numData[i][j]+numData[i+1][j]+numData[i][j+1]+numData[i+1][j+1]
                if maxSum > pre_maxSum:
                      continue
                else:
                      maxSum = pre_maxSum
    ###########   <-------------- 여기까지 코딩 (2)
